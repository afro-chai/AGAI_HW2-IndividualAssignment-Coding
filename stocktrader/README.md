# StockTrader — Individual Assignment 2 (+ bonus + capstone runway)
...
3. **Install dependencies** — From `stocktrader/`: `pip install -r requirements.txt`
...
Copy `stocktrader/.env.example` to `stocktrader/.env` or repo-root `.env` and fill keys.
...
From repository root `Individual_Assignment2` (parent of `stocktrader`):
...
Artifacts: `stocktrader/outputs/<TICKER>.json`, `summary.json`, optional `backtest.json`.
